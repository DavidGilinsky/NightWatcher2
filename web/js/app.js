...
function drawGraph(container, readings, sensor) {
  ...
  // overlay just needs the sensor's coordinates and SunCalc. Skipped silently
  ...
  if (haveEphem) {
    ...
    data.push(sunAlt, moonAlt);
    series.push(
      { label: 'sun alt°', stroke: '#ffcf5c', width: 1, dash: [5, 3], scale: 'alt', points: { show: false } },
      { label: 'moon alt°', stroke: '#c8d2ea', width: 1, dash: [5, 3], scale: 'alt', points: { show: false } },
    );
    ...
    const moonIdx = data.length - 1;  // moon altitude is the last series
  ...
  const load = async () => {
    ...
    try {
      ...
      if (kind === 'weather') {
        ...
        drawGraph(chart, rs, sensors.find(s => s.id === id));
        cols = [
          { label: 'Time (UTC)', render: r => r.ts_utc },
